    ...
    for i, noise_rate in enumerate(noise_levels):
        for trial in range(trials):
            ...
            if ("standard") in methods:
                method_num = methods.index("standard")
                ...
                relabelling_all[method_num, i, trial] = np.sum(corrected_y != y_train)
                accuracies_all[method_num, i, trial] = metrics.accuracy_score(y_test, y_pred)

            # Method 1: bootstrapped relabelling
            if ("bootstrapped") in methods:
                method_num = methods.index("bootstrapped")
            ...
            if ("control") in methods:
                method_num = methods.index("control")
