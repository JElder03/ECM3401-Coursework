...
def train(ensemble: RandomForestClassifier|ExtraTreesClassifier, X: list, y: list, labels: list, n_estimators: int = 5, iterations=10, K: float = 0.5, L: float = 0.01, B : float = 0.02, initial_certainty : float = 0.95) -> tuple[RandomForestClassifier|ExtraTreesClassifier, list]:
    """
    ...
    for _ in range(iterations):
        ...
        forest = ensemble(n_estimators=1, criterion='entropy', bootstrap=False, warm_start=True)
        X, y = shuffle(X, y)

        # Train new set of weak classifiers
        for i in range(n_estimators):
            ...
            for label in missing_labels:
                ...
                X_batch = np.vstack((X_batch, np.zeros((1, np.array(X_batch).shape[1]))))
